...
void CJsonResultManager::StoreResult(const CString& filePath, const SimpleEventData& eventData)
{
	std::lock_guard<std::mutex> lock(m_mutex);
	m_results[filePath] = eventData;
}

bool CJsonResultManager::GetResult(const CString& filePath, SimpleEventData& eventData)
{
	std::lock_guard<std::mutex> lock(m_mutex);
